...
import { 
  ...
  ArrowUpRight, User, Search, Download, 
  ArrowRight, ShieldCheck
} from 'lucide-react';

// Тот же компонент StatCard для консистентности
...
const DeanDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('submitted'); // По умолчанию показываем те, что на проверке
    ...
    <main className="border rounded-lg mx-auto px-10 py-10 bg-[#f8fafc] min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-4">
          {loading ? (
             <div className="bg-white rounded-[32px] border border-slate-100 p-20 flex flex-col items-center justify-center shadow-sm">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Загрузка протоколов...</span>
             </div>
          ) : filteredSubmissions.length > 0 ? (
            filteredSubmissions.map((row) => (
              <div key={row.user_id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-300 transition-all group relative overflow-hidden text-left">
                <div className="flex flex-col md:flex-row justify-between gap-6 relative z-10">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1 uppercase">
                          <Clock size={12}/> {row.submitted_at ? new Date(row.submitted_at).toLocaleDateString() : 'Дата не указана'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-8 min-w-[140px]">
                    <div className="text-right mb-3">
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase leading-none">план баллов</p>
                    </div>
                    
                    <button 
                      onClick={() => { setSelectedPlan(row); setIsModalOpen(true); }}
            ...
            <div className="bg-white rounded-[32px] border border-slate-200 border-dashed p-16 text-center">
               ...
               <h3 className="text-xl font-bold text-slate-900 tracking-tight">Нет планов в данной категории</h3>
               <p className="text-sm text-slate-400 mt-2">Попробуйте изменить фильтры или вкладку</p>
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm sticky top-10 text-left overflow-hidden">
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-6">
                <ShieldCheck className="text-blue-600" size={18} />
                <h4 className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-slate-400">Статистика факультета</h4>
              </div>
              <div className="space-y-8">
                <div>
                  <div className="flex justify-between items-end mb-3">
                    <div className="space-y-1">
                      <span className="text-[10px] font-bold uppercase text-slate-500 block">Планы утверждены</span>
      ...
      {/* MODAL - Переработан под стиль StaffManagement */}
          ...
          <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl relative overflow-hidden text-left border border-slate-200 flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto bg-slate-50/50 flex-1">
                <div className="space-y-4">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Комментарий к решению</p>
                    <div className="flex flex-wrap gap-2 mb-4">
                      {['План согласован', 'Требуется корректировка баллов', 'Недостаточно обоснования', 'Дубликат задач'].map((t) => (
                        <button key={t} onClick={() => setComment(t)} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold border transition-all uppercase ${comment === t ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-400'}`}>
                          {t}
                        </button>
                      ))}
                    </div>
                    <textarea 
                      ...
                      className="w-full h-32 p-4 bg-white border border-slate-200 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all resize-none"
                      placeholder="Напишите замечания или рекомендации для преподавателя..."
                    />
                </div>
            </div>
